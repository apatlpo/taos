...
def harmonic_analysis(da, constituents=[]):
    ...
    a = a.assign_coords(
        constituent=("constituent", wt.constituents()),
...
def predict_tides(
    ...
    if har is None:
        ...
        if isinstance(har, xr.Dataset):
            assert (
                realimag
            ), "You need to specify real and imaginary names via realimag kwarg"
            _har = har[realimag[0]] + 1j * har[realimag[1]]
            har = _har.assign_coords(frequency=har["frequency"]).rename(name)
        else:
            assert isinstance(
                har, xr.DataArray
            ), "har should be an xarray Dataset or DataArray"
            name = har.name
    if suffix == "":
        suffix = name + "_"
    ...
    _time = [
        (pd.Timestamp(t) - pd.Timestamp(1970, 1, 1)).total_seconds()
        for t in dsp.time.data
    ]
...
def compute_plot_ellipses(
    ...
    ds = ds.assign_coords(
        time=("time", np.arange(0, 2 * np.pi, 0.1)),
        lon=lon,
        lat=lat,
    ...
    lat_scale = 111e3
    ds["x_ref"] = (
        lon_ref + T * np.real(np.conj(u_ref) * np.exp(1j * ds.time)) / lon_scale
    )
    ds["y_ref"] = (
        lat_ref + T * np.real(np.conj(v_ref) * np.exp(1j * ds.time)) / lat_scale
    )
...
def plot_ellipses(ax, ds, markersize=1):
    ...
    ax.plot(
        ds["x"].sel(time=3 * np.pi / 2, method="nearest"),
...
def print_graph(da, name, flag):
    ...
    if not flag:
        return
    da.data.visualize(
        filename="graph_{}.png".format(name),
        optimize_graph=True,
        color="order",
        cmap="autumn",
...
def plot_bs(
    ...
    **kwargs,
    ...
    if ax is None:
        ax = fig.add_subplot(111, projection=ccrs.Orthographic(0.0, 49.5))
